...
### Video
[Video](https://vcu.mediaspace.kaltura.com/media/Deliverable+7/1_w9v8u66l)
...
FROM album
JOIN artist ON album.artist_ID = artist.ID
WHERE album.name = 'Dark Side of the Moon' AND artist.name = 'Pink Floyd';
""")
```
...
JOIN user ON playlist.user_ID = user.ID
WHERE user.user_name = 'music_lover' AND YEAR(album.release_date) = YEAR(CURRENT_DATE);
""")
```
...
FROM artist
JOIN album ON artist.ID = album.artist_ID
WHERE album.record_label = 'Universal Music';
""")
```
...
my_sql_wrapper("""
SELECT
song.name AS longest_jazz_song_name,
song.duration AS longest_jazz_song_duration
FROM song
JOIN album ON song.album_ID = album.ID
WHERE album.genre = 'jazz'
...
WHERE artist.name = 'Nicki Minaj';
""")
```

# Reflection

If I had more time, I would try to organize the code and files in a better way. Another thing that I am considering for the future is to create a website inspired by Spotify with our music database.
