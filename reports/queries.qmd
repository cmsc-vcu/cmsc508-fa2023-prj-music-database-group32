...
1. What are all songs in the key of C with the tempo of 120 bpm?
        $\pi_{name}(\sigma_{(key = C) ∧ (tempo = 120)}(song))$

2. What songs on user music_lover's "favorites" playlist were released this year?
        $\pi_{name}(\sigma_{release\_date.year}(album)(song \bowtie_{song.ID = playlist.song\_ID} (playlist\_song \bowtie (\sigma_{name = favorites}(playlist) \bowtie_{playlist.user\_ID = user.ID} \sigma_{name = music\_lover}(user)))))$

3. How many of user john_smith's followers do they follow back?
        $\pi COUNT(*) (\sigma_{user.name = john\_smith} (user \bowtie_{following.following\_ID = user.ID} (following \bowtie_{following.follower\_ID = follower.following\_ID} (\rho_{follower} (\pi_{following.follower\_ID}(\sigma_{user.name = john\_smith}(user \bowtie_{user.ID = following.follower\_ID} following)))))))$

4. What percentage of non-artist users have a premium account?
        $((\pi COUNT(*)(\sigma_{account\_type = premium}(user)))/(\pi COUNT(*)(\sigma_{ㄱ(account\_type = artist)}(user)))) * 100$

5. How many artists have released music under the "Universal Music" record label?
        $\pi COUNT(*) (\sigma_{record\_label = Universal Music}(artist \bowtie_{artist.ID = album.artist\_ID} album))$

6. What is Beyonce's least popular song?
        $\pi MIN(plays) (\sigma_{artist.name = Beyonce}(song \bowtie_{song.artistID = artist.ID} artist))$

7. What's the longest jazz song?
        $\pi MAX(song.duration) (\sigma_{genre = jazz}(song \bowtie_{song.albumID = album.ID} album))$

8. How many songs feature Nicki Minaj?
        $\pi COUNT(*) (\sigma_{artist.name = Nicki Minaj}(artist \bowtie_{artist.ID = artist\_song.artist\_ID} artist\_song)) - \pi COUNT(*) (\sigma_{artist.name = Nicki Minaj}(song \bowtie_{song.artist\_ID = artist.ID} artist))$

9. What is the description for "Bruno Mars" artist?
        $\pi_{description}(\sigma_{artist\_name = Bruno\_Mars}(ARTIST))$

10. What are the name of the every album by "Bruno Mars" artist?
        $\pi_{name}(ALBUM(\bowtie_{ALBUM.artist\_ID = ARTIST.ID}(\sigma_{artist\_name = Bruno\_Mars}(ARTIST))))$

11. What is the account type for the "music_lover" user?
        $\pi_{account\_type}(\sigma_{user\_name = music\_lover}(USER))$

12. What is the user ID for the "music_lover" user?
        $\pi_{ID}(\sigma_{user\_name = music\_lover}(USER))$

13. What is the password for the "music_lover" user?
        $\pi_{password}(\sigma_{user\_name = music\_lover}(USER))$

14. What are the name of the every playlist created by "music_lover"
         $\pi_{name}(PLAYLIST(\bowtie_{PLAYLIST.user\_ID = USER.ID}(\sigma_{user\_name = music\_lover}(USER))))$

15. What is the tempo for the "Too Good to Say Goodbye" from "24K Magic" album?
        $\pi_{tempo}(\sigma_{ALBUM.name = 24K\_Magic}((SONG)\bowtie_{SONG.album\_ID = ALBUM.ID}(ALBUM)))$
